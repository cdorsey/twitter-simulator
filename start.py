...
def start_stream():
    ...
    in name.txt) and a keyword (defined at the top of start.py) will be passed into the processing function
    :return:
    """
    auth = login.get_auth()
    api = tweepy.API(auth)
    ...
    stream.filter(track=['{0} {1}'.format(BOT_NAME, KEY_WORD)], async=True)
    print(stream.body)
